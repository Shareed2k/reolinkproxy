...
func TestServerPlayBackChannelTCP(t *testing.T) {
	backChannelReceived := make(chan struct{}, 1)
		...
		Handler: &testServerHandler{
			...
			onPlay: func(ctx *ServerHandlerOnPlayCtx) (*base.Response, error) {
				ctx.Session.OnPacketRTP(
					stream.Description().Medias[1],
					stream.Description().Medias[1].Formats[0],
					func(pkt *rtp.Packet) {
						require.Equal(t, &testRTPPacket, pkt)
						select {
						case backChannelReceived <- struct{}{}:
						default:
						}
					},
				)
	...
	stream = NewServerStream(s, &description.Session{Medias: []*description.Media{testH264Media, backMedia}})
	defer stream.Close()
	...
	require.True(t, desc.Medias[1].IsBackChannel)
	...
	doPlay(t, conn, "rtsp://127.0.0.1:8554/teststream?param=value", session)
	...
	select {
	case <-backChannelReceived:
	case <-time.After(2 * time.Second):
		t.Fatal("backchannel RTP packet was not delivered")
	}

	select {
	case <-videoSent:
	case <-time.After(2 * time.Second):
		t.Fatal("video RTP packet was not sent")
	}

	frame, err := conn.ReadInterleavedFrame()
	require.NoError(t, err)
	require.Equal(t, 0, frame.Channel)
	require.Equal(t, testRTPPacketMarshaled, frame.Payload)
}
